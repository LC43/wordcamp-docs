...
class WordCamp_Docs_Template_Speaker_Invitation implements WordCamp_Docs_Template {
	public function form( $data ) {
		...
		<div class="wcorg-speaker-invitation-form">
			<label><?php _e( 'Speaker Name:', 'wcdocs' ); ?></label>
			<input name="name" value="<?php echo esc_attr( $data['name'] ); ?>" />

			<label><?php _e( 'Gender:', 'wcdocs' ); ?></label>
			<select name="gender">
				<option value="male" <?php selected( $data['gender'], 'male' ); ?>><?php _e( 'Male', 'wcdocs' ); ?></option>
				<option value="female" <?php selected( $data['gender'], 'female' ); ?>><?php _e( 'Female', 'wcdocs' ); ?></option>
			</select>

			<label><?php _e( 'Travel Dates:', 'wcdocs' ); ?></label>
			<input name="dates" value="<?php echo esc_attr( $data['dates'] ); ?>" />
			<span class="description"><?php _e( 'Ex: January 15-16, 2019', 'wcdocs' ); ?></span>

			<label><?php _e( 'City:', 'wcdocs' ); ?></label>
			<input name="city" value="<?php echo esc_attr( $data['city'] ); ?>" />

			<label><?php _e( 'Country:', 'wcdocs' ); ?></label>
			<input name="country" value="<?php echo esc_attr( $data['country'] ); ?>" />

			<label><?php _e( 'Venue Name:', 'wcdocs' ); ?></label>
			<input name="venue_name" value="<?php echo esc_attr( $data['venue_name'] ); ?>" />

			<label><?php _e( 'Venue Address:', 'wcdocs' ); ?></label>
			<input name="venue_address" value="<?php echo esc_attr( $data['venue_address'] ); ?>" />

			<label><?php _e( 'Signature:', 'wcdocs' ); ?></label>
			<textarea name="signature"><?php echo esc_textarea( $data['signature'] ); ?></textarea>
			<span class="description"><?php _e( 'Name, title, phone, e-mail address', 'wcdocs' ); ?></span>
		</div>
...
<div class="wrap">
	...
	public function get_name() {
		return __( 'Speaker Invitation', 'wcdocs' );
	}
