...
class WordCamp_Docs {
	...
	public static function admin_menu() {
		add_menu_page( __( 'WordCamp Docs', 'wcdocs' ), __( 'Docs', 'wcdocs' ), 'manage_options', 'wcdocs', array( __CLASS__, 'render_menu_page' ), 'dashicons-portfolio', 58 );
	}
	...
	public static function render_menu_page() {
		?>
		<div class="wrap">
			<h1><?php _e( 'WordCamp Docs', 'wcdocs' ); ?></h1>
			...
			<?php if ( self::$step <= 1 ) : ?>
				...
				<p><?php _e( 'This tool will help you generate various documents and forms for your WordCamp.', 'wcdocs' ); ?></p>

				<form method="POST">
					<input type="hidden" name="wcdocs_submit" value="1" />
					<?php wp_nonce_field( 'wcdocs_step_1' ); ?>

					<p><?php _e( 'Pick a template to get started:', 'wcdocs' ); ?></p>
			...
			<?php elseif ( self::$step == 2 ) : ?>
				...
				<form method="POST">
					...
					<p class="submit">
						<input type="submit" class="button-primary" value="<?php esc_attr_e( 'Download PDF', 'wcdocs' ); ?>">
					</p>
				</form>
